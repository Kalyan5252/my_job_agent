...
export class ApplicationAgent {
  ...
  async run(
    ...
    const keepBrowserOpenAfterSubmit =
      options.keepBrowserOpenAfterSubmit ?? (mode === 'submit' && Boolean(options.preview));
    ...
    try {
      if (mode === 'submit') {
        const submitFlow = await this.browser.fillAndSubmitFormSingleSession(
          ...
          async (fields) => {
            ...
            const mappedValues = await this.llm.mapFormFields(fields, {
              profile: hydratedProfile,
              resumeSummary,
              job,
            });
            const enrichedValues = this.resumeService.enrichMappedValues(
              fields,
              mappedValues,
              hydratedProfile,
            );
        ...
        if (submitFlow.validationErrors && submitFlow.validationErrors.length > 0) {
          await this.safeUpsertApplication(
            job,
            'needs_human',
            submitFlow.validationErrors.join('; '),
          );
          return {
            status: 'needs_human',
      ...
      if (fields.length === 0) {
        ...
        await this.safeUpsertApplication(job, 'needs_human', note);
        this.log(
          `needs_human job=${job.externalId} reason=no_fields diagnosis="${diagnosis.reason}"`,
        );
        return {
          status: 'needs_human',
          message: diagnosis.reason,
      ...
      await this.safeUpsertApplication(
        job,
        'draft_filled',
        'Form filled in dry-run mode (not submitted)',
      );
